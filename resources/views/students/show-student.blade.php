        ...
        <div class="bg-white rounded-xl shadow-md overflow-hidden">
            ...
            <div class="bg-gradient-to-r from-blue-200 to-green-100 px-6 py-5 border-b border-gray-200">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        ...
                        <p class="text-sm text-gray-600 mt-1">View the details of {{ $student->full_name }}.</p>
                    </div>
            ...
            <div class="px-6 py-8 sm:px-8">
                <div class="space-y-6">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-gray-900">{{ \Carbon\Carbon::parse($student->date_of_birth)->format('d/m/Y') }}</p>
                        </div>

                        <!-- Gender -->
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Gender</label>
                            <p class="mt-1 text-gray-900">{{ ucfirst($student->gender) }}</p>
                        </div>

                        <!-- Grade -->
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Grade</label>
                            <p class="mt-1 text-gray-900">{{ $student->grade }}</p>
                        </div>

                        <!-- Enrollment Date -->
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Enrollment Date</label>
                            <p class="mt-1 text-gray-900">{{ \Carbon\Carbon::parse($student->enrollment_date)->format('d/m/Y') }}</p>
                        </div>
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-gray-900">{{ $student->guardian_phone }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex justify-end mt-8 space-x-3">
                    <a href="{{ route('students.edit', $student->id) }}" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500">
                        <svg class="h-5 w-5 mr-1" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <form action="{{ route('students.destroy', $student->id) }}" method="POST" class="inline-flex">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="inline-flex items-center px-4 py-2 bg-red-600 text-white text-sm font-medium rounded-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500" onclick="return confirm('Are you sure you want to delete this student?')">
                            <svg class="h-5 w-5 mr-1" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
